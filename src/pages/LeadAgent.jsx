...
function Glass({ children, className = "" }) {
  return (
    <div className={`border border-white/10 bg-white/[0.045] shadow-2xl shadow-black/40 backdrop-blur-2xl ${className}`}>
      {children}
    </div>
  );
}
  ...
  async function runAgent(e) {
    ...
    try {
      const response = await fetch("/api/analyze-business", {
        ...
        body: JSON.stringify({ businessName, industry, website }),
      });
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-10 mb-12 max-w-4xl">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-zinc-400">
            Detecta problemas, oportunidades de automatización y propuestas de mejora para vender sistemas AIS-01.
          </p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
          ...
          <Glass className="min-h-[520px] rounded-[2.3rem] p-6 md:p-8">
            ...
            <h2 className="text-2xl font-semibold">Lead Intelligence Report</h2>

            <div className="mt-6">
              {error && <p className="rounded-2xl border border-red-400/20 bg-red-400/10 p-4 text-red-300">{error}</p>}
              {!result && !error && <p className="pt-20 text-center text-zinc-500">El output aparecerá aquí.</p>}
              {result && (
